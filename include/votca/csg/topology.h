...
class Topology {
 public:
  ...
  Bead_T *CreateBead(byte_t symmetry, std::string bead_type, int bead_id,
                     int molecule_id,  // Don't need the molecule name that is
                                       // stored when creating molecules
                     std::string residue_type, int residue_id, double mass,
                     double charge) {
   ...
   **/
  Bead *getBead(const int index) const { return beads_[index]; }
