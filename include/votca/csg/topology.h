...
public:
   ...
   */
  virtual std::shared_ptr<Bead> CreateBead(byte_t symmetry, string name,
  ...
  */
   ...
   */
  void CreateMoleculesByRange(string name, int first, int nbeads,
                              int nmolecules);
  ...
  std::list<std::shared_ptr<Interaction>>
  InteractionsInGroup(const string &group);

  std::shared_ptr<BeadType> getBeadType(const int i) const {
    return _beadtypes[i];
  }
  std::shared_ptr<Bead> getBead(const int i) const { return _beads[i]; }
  std::shared_ptr<Residue> getResidue(const int i) const {
    return _residues[i];
  }
  std::shared_ptr<Molecule> getMolecule(const int i) const {
    return _molecules[i];
  }
   ...
   */
  void setParticleGroup(string particle_group) {
   ...
   */
  string getParticleGroup() { return _particle_group; };
...
protected:
  ...
  std::map<std::string, std::list<std::shared_ptr<Interaction>>>
