...
unordered_map<int, string> CGMoleculeStencil::MapCGBeadIdsToCGBeadNames(
  ...
  assert(bead_ids.size() == bead_info_.size() &&
         "when mapping cg bead ids to cg bead names you must have the same "
         "number of beads in the molcule as in the stencil");
  sort(bead_ids.begin(), bead_ids.end());
...
void CGMoleculeStencil::AddInteractionStencil(
    const vector<CGInteractionStencil> &interaction_info) {
  interaction_info_ = interaction_info;
}

const vector<CGBeadStencil> &CGMoleculeStencil::getBeadStencil() const {
  return bead_info_;
}

const vector<CGInteractionStencil> &CGMoleculeStencil::getInteractionStencil()
    const {
  return interaction_info_;
}

const std::string &CGMoleculeStencil::getCGMoleculeType() const {
  return cg_molecule_type_;
}
