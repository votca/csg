...
namespace votca {
  ...
  bool DLPOLYTrajectoryReader::NextFrame(CSG_Topology &conf) {
    ...
    if (_first_frame) {
      ...
      hasVs = (mavecs > 0);  // 1 or 2 => in DL_POLY frame velocity vector
      ...
      if (static_cast<size_t>(matoms) != conf.BeadCount())
        throw std::runtime_error(
            "Number of atoms/beads in '" + _fname +
      ...
      if (_isConfig) {
        ...
        if (std::abs(stime - conf.getTime()) > 1.e-8) {
          ...
          if (nerrt < 11) {
            ...
            // cout << "Check: nstep = " << nstep << ", dt = " << dtime << ",
            // time = " << conf.getTime() << " (correct?)" << endl;
          } else if (nerrt == 11) {
...
#ifdef DEBUG
        cout << "Crds from dlpoly file '" << _fname
             << "' : " << atom_vecs.col(0) << endl;
#endif
        if (navecs > 0) {
